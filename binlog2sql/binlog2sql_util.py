...
def parse_args(args):
    ...
    parser.add_argument('--stop-never', dest='stopnever', action='store_true',
                        help='Wait for more data from the server. default: stop replicate at the last binlog when you start binlog2sql',
                        default=False)
...
def concat_sql_from_binlogevent(cursor, binlogevent, row=None, eStartPos=None, flashback=False, nopk=False,
    ...
    if flashback and nopk:
        raise ValueError('only one of flashback or nopk can be True')
    if not (isinstance(binlogevent, WriteRowsEvent) or isinstance(binlogevent, UpdateRowsEvent) or isinstance(
            binlogevent, DeleteRowsEvent) or isinstance(binlogevent, QueryEvent)):
        raise ValueError('binlogevent must be WriteRowsEvent, UpdateRowsEvent, DeleteRowsEvent or QueryEvent')

    sql = ''
    if isinstance(binlogevent, WriteRowsEvent) or isinstance(binlogevent, UpdateRowsEvent) or isinstance(binlogevent,
                                                                                                         DeleteRowsEvent):
        pattern = generate_sql_pattern(binlogevent, row=row, flashback=flashback, nopk=nopk, pk_list=pk_list)
        sql = cursor.mogrify(pattern['template'], pattern['values'])
        sql += ' #start %s end %s time %s' % (
            eStartPos, binlogevent.packet.log_pos, datetime.datetime.fromtimestamp(binlogevent.timestamp))
    elif flashback is False and isinstance(binlogevent,
                                           QueryEvent) and binlogevent.query != 'BEGIN' and binlogevent.query != 'COMMIT':
...
def generate_sql_pattern(binlogevent, row=None, flashback=False, nopk=False, pk_list=None):
    template = ''
    values = []
    if flashback is True:
        if isinstance(binlogevent, WriteRowsEvent):
            if binlogevent.schema + '.' + binlogevent.table in pk_list.keys():
                template = 'DELETE FROM `{0}`.`{1}` WHERE {2} LIMIT 1;'.format(
                    binlogevent.schema, binlogevent.table,
                    ' AND '.join(map(compare_items1, pk_list[binlogevent.schema + '.' + binlogevent.table]))
                )
                values = map(lambda x: row['values'][x], pk_list[binlogevent.schema + '.' + binlogevent.table])
            else:
                template = 'DELETE FROM `{0}`.`{1}` WHERE {2} LIMIT 1;'.format(
                    binlogevent.schema, binlogevent.table,
            ...
            if binlogevent.schema + '.' + binlogevent.table in pk_list.keys():
                template = 'UPDATE `{0}`.`{1}` SET {2} WHERE {3} LIMIT 1;'.format(
                    ...
                    ' AND '.join(map(compare_items1, pk_list[binlogevent.schema + '.' + binlogevent.table])))
                where_value_list = map(lambda x: row['after_values'][x],
                                       pk_list[binlogevent.schema + '.' + binlogevent.table])
                values = map(fix_object, row['before_values'].values() + where_value_list)
            else:
                template = 'UPDATE `{0}`.`{1}` SET {2} WHERE {3} LIMIT 1;'.format(
                    binlogevent.schema, binlogevent.table,
                    ', '.join(['`%s`=%%s' % k for k in row['before_values'].keys()]),
